...
const MediumArticles = () => {
    ...
    useEffect(() => {
        fetch('https://api.rss2json.com/v1/api.json?rss_url=https://medium.com/feed/@mjhossainnyc')
            ...
            .then(data => {
                const mediumArticles = data.items; // Assuming the items array contains your articles
            ...
            {articles.map((article, index) => {
                ...
                return (
                    <div key={index} className='articleCard'>
                        <img
                            src={imgLink} // Thumbnail might be available depending on the RSS conversion
                            alt={article.title}
                        />
                        <h3>{article.title}</h3>
                    </div>
